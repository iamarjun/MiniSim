...
class RestartCommand: Action {
  let device: Device

  init(device: Device) { self.device = device }

  func execute() throws {
    guard let id = device.identifier else { throw DeviceError.deviceNotFound }
    let adbPath = try ADB.getAdbPath()
    try Shell().execute(command: "\(adbPath) -s \(id) reboot")
  }
}
...
class TakeScreenshotAction: Action {
  ...
  func execute() throws {
    ...
    let shell = Shell()
    let adbPath = try ADB.getAdbPath()
    let remote = "/sdcard/androidbar_ss.png"
    let local = "\(NSHomeDirectory())/Desktop/screenshot_\(device.name)_\(Int(Date().timeIntervalSince1970)).png"
    try shell.execute(command: "\(adbPath) -s \(id) shell screencap -p \(remote)")
    try shell.execute(command: "\(adbPath) -s \(id) pull \(remote) \(Shell.escapeShellArgument(local))")
    try shell.execute(command: "\(adbPath) -s \(id) shell rm \(remote)")
    DispatchQueue.main.async {
      NSWorkspace.shared.open(URL(fileURLWithPath: local))
    }
    AndroidBar.showSuccessMessage(title: "Screenshot saved!", message: "Saved to Desktop.")
  }
}

class RecordScreenAction: Action {
  let device: Device
  ...
  func execute() throws {
    ...
    let remote = "/sdcard/androidbar_rec_\(ts).mp4"
    let local = "\(NSHomeDirectory())/Desktop/recording_\(ts).mp4"
    let localEsc = Shell.escapeShellArgument(local)
    // Record until Ctrl+C, then pull and open
    let cmd = "\(adbPath) -s \(id) shell screenrecord \(remote); \(adbPath) -s \(id) pull \(remote) \(localEsc) && \(adbPath) -s \(id) shell rm \(remote) && open \(localEsc)"
    try TerminalService.launchTerminal(command: cmd)
  }
}

class OpenInStudioAction: Action {
  let device: Device

  init(device: Device) { self.device = device }

  func execute() throws {
    try Shell().execute(command: "open -a 'Android Studio'")
  }
}

class WipeDataAction: Action {
  let device: Device
  let skipConfirmation: Bool
  ...
  func showQuestionDialog() -> Bool {
    ...
    return !NSAlert.showQuestionDialog(
      title: "Wipe data?",
      message: "All user data on \"\(device.name)\" will be erased. The emulator will restart."
    )
  }

  func execute() throws {
    if device.booted, let id = device.identifier {
      let adbPath = try ADB.getAdbPath()
      try Shell().execute(command: "\(adbPath) -s \(id) emu kill")
      Thread.sleep(forTimeInterval: 1.0)
    }
    try device.launch(additionalArgs: ["-wipe-data"])
  }
}
